		...
		>
			{tabs.map((tab) => {
				...
				return (
					...
					>
						<Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${isActive ? "text-gray-900 dark:text-white" : "text-gray-900 dark:text-gray-400"}`} />
						{isActive ? (
							<span className="inline text-gray-900 dark:text-white">{label}</span>
						) : (
							<span className="hidden sm:inline text-gray-900 dark:text-gray-400">{label}</span>
						)}
						...
						{isActive && (
							<motion.div
								layoutId="activeTab"
								className="absolute inset-0 rounded-lg bg-green-100 dark:bg-gradient-to-r dark:from-green-500 dark:to-emerald-500 shadow-lg shadow-green-500/30 dark:shadow-green-500/20 -z-10"
