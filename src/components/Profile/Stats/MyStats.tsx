        ...
        <div className="min-h-screen bg-gray-50" dir={direction}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
                        <div className={`flex items-center gap-3 ${isArabic ? 'flex-row' : 'flex-row'}`}>
                            ...
                            <div className={isArabic ? 'text-right' : 'text-left'}>
                                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">إحصائياتي</h1>
                                <p className="text-gray-600 text-sm">نظرة عامة على أرصدتك ونشاطك</p>
                            </div>
                        </div>
                    </div>
                </div>
			...
			{/* إحصائيات المالية */}
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    <div className="flex items-center justify-between">
                        <div className={`flex flex-col ${isArabic ? 'items-end' : 'items-start'}`}>
                            <span className="text-xs sm:text-sm text-gray-500 mb-1">المبلغ المتاح</span>
                        ...
                        <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-green-100">
                            <FaWallet className="text-green-600" size={22} />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    <div className="flex items-center justify-between">
                        <div className={`flex flex-col ${isArabic ? 'items-end' : 'items-start'}`}>
                            <span className="text-xs sm:text-sm text-gray-500 mb-1">قيدها</span>
                        ...
                        <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-orange-100">
                            <FaWallet className="text-orange-600" size={22} />
                        </div>
                    </div>
                </div>
                </div>
			...
			{/* المنتجات الأكثر شراءً */}
                ...
                <h3 className={`${isArabic ? 'text-right' : 'text-left'} text-lg sm:text-xl font-bold text-gray-800 mb-4 sm:mb-6`}>
					المنتجات الأكثر شراءً
				</h3>
				
				{mostPurchasedProducts.length === 0 ? (
                    <div className="text-center py-6 sm:py-8">
                        <p className="text-sm sm:text-base text-gray-500">لا توجد منتجات مشتراة بعد</p>
					</div>
				) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-6">
						{mostPurchasedProducts.map((product) => (
                            <div key={product.id} className={`flex items-center ${isArabic ? 'space-x-reverse space-x-3' : 'space-x-3'} p-3 border border-gray-200 rounded-lg`}>
								<img
									src={product.image}
									alt={product.name}
								...
								<div className="flex-1">
                                    <h4 className="font-semibold text-gray-800 text-xs sm:text-sm">{product.name}</h4>
                                    <div className={`flex items-center ${isArabic ? 'space-x-reverse' : ''} space-x-2 mt-1`}>
										...
										{product.originalPrice && (
                                            <span className="text-gray-400 line-through text-[10px] sm:text-xs">{product.originalPrice}</span>
										)}
									</div>
                                    <span className="text-gray-500 text-[10px] sm:text-xs">{product.unit}</span>
								</div>
							</div>
						))}
					</div>
			...
			{/* إحصائيات إضافية */}
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 text-center">
                    ...
                    <div className="text-gray-600 text-sm sm:text-base">إجمالي الطلبات</div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 text-center">
                    <div className="text-2xl sm:text-3xl font-bold text-purple-600 mb-1 sm:mb-2">0</div>
                    <div className="text-gray-600 text-sm sm:text-base">المتاجر المفضلة</div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 text-center">
                    <div className="text-2xl sm:text-3xl font-bold text-green-600 mb-1 sm:mb-2">0</div>
                    <div className="text-gray-600 text-sm sm:text-base">النقاط المكتسبة</div>
                </div>
                </div>
            </div>
        </div>
