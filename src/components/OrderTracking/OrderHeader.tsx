...
export default function OrderHeader({
	...
	return (
		<motion.div
			...
			className="bg-white dark:bg-[#1B1D22] rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 p-4 sm:p-6 mb-4 sm:mb-6"
		>
			<div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 ${isArabic ? "sm:flex-row-reverse" : ""}`}>
				...
				<div className={`flex items-start gap-3 sm:gap-4 flex-1 min-w-0 ${isArabic ? "flex-row-reverse" : ""}`}>
					...
					<div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${
						...
						{type === "service" ? (
							<Wrench className="w-5 h-5 sm:w-6 sm:h-6 text-orange-600 dark:text-orange-400" />
						) : (
					...
					<div className="flex-1 min-w-0">
						<div className={`flex items-center gap-2 sm:gap-3 mb-1.5 sm:mb-2 flex-wrap ${isArabic ? "flex-row-reverse" : ""}`}>
							...
							<StatusBadge status={status} language={language} type={type} />
						</div>
						<div className={`flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 md:gap-4 ${isArabic ? "sm:flex-row-reverse" : ""}`}>
							<p className={`text-xs sm:text-sm text-gray-600 dark:text-gray-400 truncate ${isArabic ? "text-right" : "text-left"}`}>
								<span className="font-semibold">{isArabic ? "رقم الطلب:" : "Order #:"}</span> <span className="font-mono">{orderId}</span>
							</p>
							<span className="hidden sm:inline text-gray-300 dark:text-gray-600">•</span>
							<span className={`px-2 sm:px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md sm:rounded-lg text-xs font-semibold whitespace-nowrap ${isArabic ? "text-right" : "text-left"}`}>
								{getTypeLabel()}
							</span>
						</div>
					</div>
